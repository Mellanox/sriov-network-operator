...
var _ = Describe("DRA cleanup without optional APIs", func() {
	It("allows device-plugin mode when the REST mapper cannot discover DRA CRDs", func() {
		// Use the envtest client configuration with a mapper representing an
		// installation that has not installed the optional DRA CRDs.
		client, err := k8sclient.New(cfg, k8sclient.Options{
			Scheme: k8sClient.Scheme(),
			Mapper: meta.NewDefaultRESTMapper([]schema.GroupVersion{}),
		})
		Expect(err).NotTo(HaveOccurred())
		reconciler := &SriovNetworkNodePolicyReconciler{Client: client}
		Expect(reconciler.cleanupSriovResourcePoliciesAndDeviceAttributes(context.Background())).To(Succeed())
	})
})
